...
/**
 * Defines a memory database {@linkcode Artist} items test
 */
export class ArtistRepository {
  private static validateProperty(
    artist: Map<string, Lengthwise>,
    key: string
  ): string | null {
  ...
  private static validate(artist: Artist): void | never {
    ...
    const errors = REQUIRED_ARTIST_PROPERTIES.map((key) =>
      ArtistRepository.validateProperty(artistMap, key)
    ).filter(Boolean);
  ...
  static insert(artist: Artist): void | never {
    ...
    if (dbArtist) throw new Error("Artist already exists!");
    artists.push(artist);
  }
  ...
  static update(artist: Artist): void | never {
    ...
    const artistIndex = artists.findIndex(
      (dbArtist) => dbArtist.id === artist.id
    );
